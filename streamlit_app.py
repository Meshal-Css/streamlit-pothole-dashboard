...
def heatmap_page():
    ...
    new_file_path = 'potholes road detection _location_data.csv'
    ...
    if 'Location' in new_pothole_data.columns:
        ...
        st.write("Latitude and Longitude columns created.")
        
        new_pothole_data['Pothole_ID'] = new_pothole_data.index + 1

        new_entry = pd.DataFrame([{
            'Pothole_ID': new_pothole_data['Pothole_ID'].max() + 1,
        ...
        new_pothole_data = pd.concat([new_pothole_data, new_entry], ignore_index=True)

        valid_data = new_pothole_data.dropna(subset=['Latitude', 'Longitude'])
        
        st.write("Valid data processed successfully.")
